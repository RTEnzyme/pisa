...
class persistent_freq_index {
public:
    using index_layout_tag = PMIndexTag;

    persistent_freq_index() 
    : m_docs_sequences(PM_AS_DRAM), m_freqs_sequences(PM_AS_DRAM) {
    }
     ...
     * 
    */
   explicit persistent_freq_index(MemorySource source) 
   : m_docs_sequences(PM_AS_DRAM), m_freqs_sequences(PM_AS_DRAM),m_source(std::move(source)) 
   {
 ...
 [[nodiscard]] std::uint64_t num_docs() const noexcept { return m_num_docs; }
    ...
    class document_enumerator {
        ...
        private:
        friend class persistent_freq_index;
    ...
    class builder {
      public:
         ...
         */
        void build(persistent_freq_index& sq)
        {
  ...
  */
 void swap(persistent_freq_index& other) {
